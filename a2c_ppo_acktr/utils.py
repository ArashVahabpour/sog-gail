...
def generate_latent_codes(args, count=None, vae_data=None, eval=False):
    ...
    if eval:
        if args.vae_gail:
            ...
            else:  # grids will be prohibitively large 
                return torch.randn(count, args.latent_dim, device=args.device)
        ...
        if count is None:
            count = 1
        if args.vae_gail:
            idx = np.random.randint(0, vae_codes.size(0), count)
            return vae_codes[idx]
        elif args.continuous:
